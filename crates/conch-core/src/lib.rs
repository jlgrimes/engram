...
pub use consolidate::{consolidate, find_clusters, ConsolidateCluster, ConsolidateResult};
pub use decay::{run_decay, DecayResult};
pub use embed::{cosine_similarity, EmbedError, Embedder, FastEmbedder, SharedEmbedder};
pub use importance::{
    compute_importance, list_importance, score_all as score_importance, ImportanceInfo,
};
pub use isomorphic::{
    isomorphic_recall, IsomorphicRecallResult, IsomorphicResult, RetrievalSource,
...
impl ConchDB {
    ...
    pub fn open_path_with_embedder(
        path: &str,
        embedder: Box<dyn Embedder>,
        namespace: &str,
    ) -> Result<Self, ConchError> {
        let store = MemoryStore::open(path)?;
        Ok(Self {
            store,
            embedder,
    ...
    pub fn open_in_memory_with_namespace(
        embedder: Box<dyn Embedder>,
        namespace: &str,
    ) -> Result<Self, ConchError> {
        ...
        if let Some(config) = &self.validation_config {
            ...
            if !result.passed {
                let violations: Vec<String> =
                    result.violations.iter().map(|v| format!("{v:?}")).collect();
    ...
    pub fn remember_fact_full(
        ...
        let id = self.store.remember_fact_ns(
            subject,
            relation,
            object,
            Some(&embedding),
    ...
    pub fn upsert_fact_with_tags(
        ...
        Ok((
            self.store.get_memory(id)?.expect("just upserted"),
            was_updated,
        ))
    }
    ...
    pub fn remember_episode_full(
        ...
        let embedding = self.embedder.embed_one(text)?;
        let id = self.store.remember_episode_ns(
            text,
            Some(&embedding),
            tags,
            source,
    ...
    pub fn remember_action(&self, text: &str) -> Result<MemoryRecord, ConchError> {
        self.remember_action_with_tags(text, &[])
    }
    ...
    pub fn remember_action_with_tags(
        ...
        self.remember_action_full(text, tags, None, None, None)
    }
    ...
    fn find_duplicate_excluding(
        &self,
        embedding: &[f32],
        exclude_id: i64,
    ) -> Result<Option<(i64, f32)>, ConchError> {
    ...
    pub fn remember_fact_dedup_with_tags(
        &self,
        subject: &str,
        relation: &str,
        object: &str,
    ...
    pub fn remember_fact_dedup_full(
        ...
        // Fast path: exact-content dedup by checksum avoids embedding generation and
        // full similarity scans for replayed writes/retries.
        let checksum = crate::store::content_checksum(&text);
        if let Some(existing) = self
            .store
            .find_fact_by_checksum_ns(&checksum, &self.namespace)?
        {
            self.store
                .reinforce_memory(existing.id, Self::DEDUP_REINFORCE_BOOST)?;
            let updated = self
                .store
                .get_memory(existing.id)?
                .expect("just reinforced");
            return Ok(RememberResult::Duplicate {
                existing: updated,
                similarity: 1.0,
            });
        }
        ...
        if let Some((existing_id, similarity)) = self.find_duplicate_excluding(&embedding, id)? {
            ...
            self.store
                .reinforce_memory(existing_id, Self::DEDUP_REINFORCE_BOOST)?;
            let existing = self
                .store
    ...
    pub fn remember_episode_dedup_with_tags(
        &self,
        text: &str,
        tags: &[String],
    ) -> Result<RememberResult, ConchError> {
    ...
    pub fn remember_episode_dedup_full(
        ...
        // Fast path: exact-content dedup by checksum avoids an embedding call and
        // full similarity scan for common replay/retry writes.
        let checksum = crate::store::content_checksum(text);
        if let Some(existing) = self
            .store
            .find_memory_by_checksum_ns(&checksum, &self.namespace)?
        {
            self.store
                .reinforce_memory(existing.id, Self::DEDUP_REINFORCE_BOOST)?;
            let updated = self
    ...
    pub fn recall_with_tag(
        &self,
        query: &str,
        limit: usize,
        tag: Option<&str>,
    ) -> Result<Vec<RecallResult>, ConchError> {
        recall::recall_with_tag_filter_ns(
            &self.store,
            query,
    ...
    pub fn forget_older_than(&self, secs: i64) -> Result<usize, ConchError> {
        ...
        Ok(self
            .store
            .forget_older_than_ns(Duration::seconds(secs), &self.namespace)?)
    }

    pub fn decay(&self) -> Result<DecayResult, ConchError> {
        Ok(decay::run_decay_ns(
            &self.store,
            None,
            None,
            &self.namespace,
    ...
    pub fn audit_log(
        ...
        actor: Option<&str>,
    ) -> Result<Vec<AuditEntry>, ConchError> {
...
mod tests {
    ...
    fn dedup_detects_identical_embedding() {
        // All texts produce the same embedding. Exact replay should short-circuit
        // to duplicate via checksum and avoid adding a second row.
        ...
        assert!(r2.is_duplicate(), "second identical fact should dedup");
    ...
    fn dedup_detects_identical_episode_embedding() {
        ...
        let r1 = db
            .remember_episode_dedup("Meeting notes from standup")
            .unwrap();
        assert!(!r1.is_duplicate(), "first insert should not be duplicate");
        ...
        assert!(
            r2.is_duplicate(),
            "second identical episode should be duplicate"
        );

        if let RememberResult::Duplicate { similarity, .. } = r2 {
            assert!(
                similarity > 0.95,
                "similarity should be > 0.95, got {similarity}"
            );
        }
    ...
    #[test]
    fn dedup_exact_episode_checksum_short_circuits_similarity_scan() {
        let db = ConchDB::open_in_memory_with(Box::new(OrthogonalEmbedder::new())).unwrap();

        let r1 = db
            .remember_episode_dedup("Repeated webhook payload")
            .unwrap();
        ...
        if let RememberResult::Duplicate { similarity, .. } = r2 {
            assert!(
                ...
                "checksum fast-path should emit similarity=1.0"
            );
        }
        ...
        assert_eq!(
            ...
            "exact replay should not create a second row"
        );
    }

    #[test]
    fn dedup_exact_fact_checksum_short_circuits_embedding() {
        let calls = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let db = ConchDB::open_in_memory_with(Box::new(CountingEmbedder {
            calls: calls.clone(),
        }))
        .unwrap();
        ...
        assert_eq!(
            ...
            1,
            "second exact write should bypass embedding via checksum fast-path"
        );
    }
    ...
    fn dedup_allows_different_memories() {
        ...
        let r2 = db
            .remember_episode_dedup("Had coffee this morning")
            .unwrap();
        assert!(!r2.is_duplicate());
    ...
    fn dedup_reinforces_decayed_memory() {
        ...
        let r1 = db
            .remember_episode_dedup("Important project context")
            .unwrap();
        let id = r1.memory().id;
        ...
        let reinforced = r2.memory();
        assert!(
            (reinforced.strength - 0.6).abs() < 0.01,
            "strength should be ~0.6 after reinforcement, got {}",
            reinforced.strength
        );
    }
    ...
    fn dedup_episode_detected_as_duplicate_of_fact() {
        ...
        assert!(
            r2.is_duplicate(),
            "episode matching a fact should be detected as duplicate"
        );
    ...
    fn dedup_with_empty_db_always_creates() {
        ...
        assert!(
            !r1.is_duplicate(),
            "first memory in empty DB should always be created"
        );
    }
    ...
    fn store_all_embeddings_returns_correct_count() {
        ...
        store
            .remember_fact("A", "B", "C", Some(&[1.0, 0.0]))
            .unwrap();
    ...
    fn store_reinforce_memory_clamps_to_1() {
        ...
        let mem = store.get_memory(id).unwrap().unwrap();
        assert!(
            (mem.strength - 1.0).abs() < f64::EPSILON,
    ...
    fn upsert_via_dedup_updates_existing_fact() {
        ...
        let r1 = db
            .remember_fact_dedup_full("Jared", "favorite_color", "blue", &[], None, None, None)
            .unwrap();
        assert!(!r1.is_duplicate());
        assert!(!r1.is_updated());

        let r2 = db
            .remember_fact_dedup_full("Jared", "favorite_color", "green", &[], None, None, None)
            .unwrap();
        assert!(
            r2.is_updated(),
            "same subject+relation should trigger upsert"
        );
    ...
    fn upsert_different_subject_creates_new() {
        ...
        db.remember_fact_dedup_full("Jared", "likes", "Rust", &[], None, None, None)
            .unwrap();
        let r2 = db
            .remember_fact_dedup_full("Alice", "likes", "Python", &[], None, None, None)
            .unwrap();
        assert!(
            !r2.is_updated(),
            "different subject should not trigger upsert"
        );
        assert!(
            !r2.is_duplicate(),
            "orthogonal embeddings should not trigger dedup"
        );
    ...
    fn related_finds_direct_connections() {
        ...
        assert_eq!(
            nodes.len(),
            2,
            "Alice should have 2 direct connections, got {}",
            nodes.len()
        );
        for node in &nodes {
            assert_eq!(
                node.depth, 0,
                "all nodes at depth 1 traversal should be hop 0"
            );
        }
    }
    ...
    fn related_no_duplicates() {
        ...
        assert_eq!(
            ids.len(),
            unique.len(),
            "should have no duplicate memory IDs"
        );
    }
    ...
    fn related_finds_reverse_connections() {
        ...
        assert!(
            nodes.len() >= 2,
            "Bob should be found as both subject and object, got {}",
            nodes.len()
        );
    }
    ...
    fn why_episode_has_no_related() {
        ...
        assert!(
            info.related.is_empty(),
            "episodes should have no graph-related facts"
        );
    }
    ...
    fn namespace_isolation_facts() {
        ...
        let store = MemoryStore::open_in_memory().unwrap();
        store
            .remember_fact_ns("X", "is", "A", None, &[], None, None, None, "ns-a")
            .unwrap();
        store
            .remember_fact_ns("Y", "is", "B", None, &[], None, None, None, "ns-b")
            .unwrap();
        ...
        assert_eq!(
            stats_default.total_memories, 0,
            "default namespace should be empty"
        );
    ...
    fn namespace_upsert_scoped() {
        ...
        store
            ...
            .unwrap();
        store
            .upsert_fact_ns("Jared", "color", "red", None, &[], None, None, None, "ns-b")
            .unwrap();
        ...
        if let MemoryKind::Fact(f) = &all_a[0].kind {
            assert_eq!(f.object, "blue");
        } else {
            panic!();
        }
        if let MemoryKind::Fact(f) = &all_b[0].kind {
            assert_eq!(f.object, "red");
        } else {
            panic!();
        }
    ...
    fn audit_log_records_remember() {
        ...
        assert!(
            log.iter().any(|e| e.action == "remember"),
            "should have a remember action"
        );
    }
    ...
    fn audit_log_records_forget() {
        ...
        let log = db.audit_log(10, None, None).unwrap();
        assert!(
            log.iter().any(|e| e.action == "forget"),
            "should have a forget action"
        );
    }
    ...
    fn regression_forget_older_than_negative_secs_is_rejected() {
        ...
        db.remember_episode("important context that must survive")
            .unwrap();
        ...
        assert_eq!(
            ...
            "no memories should have been deleted"
        );
    }
    ...
    fn regression_forget_older_than_zero_secs_is_rejected() {
        ...
        assert_eq!(
            stats.total_memories, 1,
            "memory must survive a rejected forget call"
        );
    }
    ...
    fn recall_stored_fact_has_no_false_negative() {
        ...
        assert!(
            found,
            "the stored fact must appear in recall results — false negative is a P1 bug"
        );
    }
    ...
    fn recall_stored_episode_has_no_false_negative() {
        let db = ConchDB::open_in_memory_with(Box::new(IdenticalEmbedder)).unwrap();
        db.remember_episode("Jared submitted Gen to Y Combinator")
            .unwrap();
        ...
        let found = results.iter().any(
            |r| matches!(&r.memory.kind, MemoryKind::Episode(e) if e.text.contains("Y Combinator")),
        );
        assert!(
            found,
            "the stored episode must appear in recall results — false negative is a P1 bug"
        );
    }
    ...
    fn export_import_round_trip_preserves_count() {
        ...
        source
            .remember_episode("Shipped conch v0.2 with 117 tests")
            .unwrap();
    ...
    fn export_import_round_trip_preserves_field_values() {
        let source = ConchDB::open_in_memory_with(Box::new(OrthogonalEmbedder::new())).unwrap();
        source
            .remember_fact("Jared", "works_at", "Microsoft")
            .unwrap();
        source
            .remember_episode("Tortellini is Jared's dog")
            .unwrap();
        ...
        let fact = all
            .iter()
            .find(|m| matches!(&m.kind, MemoryKind::Fact(f) if f.subject == "Jared"))
            .unwrap();
        let episode = all
            .iter()
            .find(|m| matches!(&m.kind, MemoryKind::Episode(e) if e.text.contains("Tortellini")))
            .unwrap();
        ...
        if let MemoryKind::Episode(e) = &episode.kind {
            assert!(
                e.text.contains("Tortellini"),
                "episode text must survive round-trip"
            );
        }
        // Strength must survive (default is 1.0)
        assert!(
            (fact.strength - 1.0).abs() < f64::EPSILON,
            "strength must be preserved through export/import"
        );
    }
}
